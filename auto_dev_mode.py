...
def ejecutar_desarrollo_total():
    # 1. DB SYNC (Inspiración BlaBlaCar)
    db_path = 'portal-web/src/app/rutas.json'
    db_data = [
        {"id": "V1", "status": "verified", "origin": "Madrid", "dest": "Londres", "price": 45, "user": "Alex G.", "stars": 4.9, "size": "M"},
        {"id": "V2", "status": "active", "origin": "Barcelona", "dest": "París", "price": 30, "user": "Sara L.", "stars": 5.0, "size": "S"}
    ]
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    with open(db_path, 'w') as f:
        json.dump(db_data, f, indent=4)

    # 2. UI INJECTION (Inspiración Uber/Wallapop)
    page_content = """
...
"""
    with open('portal-web/src/app/page.tsx', 'w', encoding='utf-8') as f:
        f.write(page_content)
    print('✅ [VALENTINA]: Interfaz de Mercado desplegada sin errores.')

if __name__ == '__main__':
    ejecutar_desarrollo_total()
